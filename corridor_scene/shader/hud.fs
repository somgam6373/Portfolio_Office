...
void main()
{
    ...
    // sRGB → linear
    vec3 diffTex = pow(diffuseSample.rgb, vec3(2.2));
    vec3 specTex = vec3(0.35);  // default specular (moon stick has no separate specular map)
    ...
    // directional light with PCF shadow
    ...
    // linear → sRGB (HUD renders directly to default framebuffer, not HDR FBO)
